...
from utils_ import HiddenPrints, serialize_to_dict
...
class GPT3Model(BaseModel):
    ...
    def query_gpt3(self, prompt, model="gpt-3.5-turbo", max_tokens=16, logprobs=None, stream=False,
        ...
        if 'gpt' in model:
            ...
            response = serialize_to_dict(openai_client.chat.completions.create(
            ...
            response = serialize_to_dict(openai_client.completions.create(
                model=model,
                prompt=prompt,
                max_tokens=max_tokens,
                logprobs=logprobs,
...
class BLIPModel(BaseModel):
    ...
    def __init__(self, gpu_number=0, half_precision=config.blip_half_precision,
        ...
        with warnings.catch_warnings(), HiddenPrints("BLIP"), torch.cuda.device(self.dev):
            ...
            self.processor = Blip2Processor.from_pretrained(f"Salesforce/{blip_v2_model_type}", local_files_only=False)
            # Device_map must be sequential for manual GPU selection
            try:
                # print(f"Loading BLIP model {blip_v2_model_type} loaded in 8bit: {half_precision} \n ---------------------")
                # blip_v2_model_type = 'blip2-flan-t5-xl'

                print('blip_v2_model_type', blip_v2_model_type)
                self.model = Blip2ForConditionalGeneration.from_pretrained(
                    ...
                    device_map="sequential", max_memory=max_memory, local_files_only=False
                )
            except Exception as e:
    ...
    def qa(self, image, question):
        ...
        # print(inputs.input_ids.shape, "qa")   
        if self.half_precision:
            inputs['pixel_values'] = inputs['pixel_values'].half()
        generated_ids = self.model.generate(input_ids=inputs.input_ids, pixel_values=inputs['pixel_values'], attention_mask=inputs.attention_mask, length_penalty=-1, num_beams=5, max_new_tokens=10, min_length=1,
                                            do_sample=False, top_p=0.9, repetition_penalty=1.0,
                                            num_return_sequences=1, temperature=1)
