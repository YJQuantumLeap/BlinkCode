...
from utils_ import show_single_image, load_json, create_tempdir
...
def Ocr_by_gpt4o(image_path):
    headers = {
        "Content-Type": "application/json",
        "Authorization": f"Bearer {OPENAI_KEY}"
    }
    question = "Write down all texts in this figure in raw markdown format showing the original code, without any processing, shown in only one code block. The texts should be described from the upper-left to the bottom-right. All variables, formulas and words with subscripts or superscripts should be shown in latex form wrapped in '$'. You should not provide any explanation for your return, just return the corresponding information from the image.If there is no text information, return None directly."
    ...
    if "error" in response or response == {}:
        if "Rate limit" in response["error"]["message"]:
            time.sleep(20)
            return Ocr_by_gpt4o(image_path)
        else:
...
class ImagePatch:
    ...
    def find(self, object_name: str) -> list[ImagePatch]:
        ...
        if all_object_coordinates == None or len(all_object_coordinates) == 0:
            return []
    ...
    def OCR(self) -> None:
        """
        Save the cropped image locally for OCR processing.
        """
        pil_image = transforms.ToPILImage()(self.cropped_image)
        content = ''
        with create_tempdir() as tempdir:
            save_path = tempdir + '0.png'
            pil_image.save(save_path)
            result, content = Ocr_by_gpt4o(save_path)
            if result == False:
